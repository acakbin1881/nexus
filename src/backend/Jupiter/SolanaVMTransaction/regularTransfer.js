...
async function transferSPLToken() {
    ...
    const recipientATA = await getOrCreateAssociatedTokenAccount(connection, sender, // payer
    mint, RECIPIENT);
    console.log("Recipient ATA:", recipientATA.address.toBase58());
    const TRANSFER_AMOUNT = 1000000n; // adjust decimals to match your mint
    const signature = await transfer(connection, sender, senderATA.address, recipientATA.address, sender, TRANSFER_AMOUNT);
    console.log("Signature:", signature);
    console.log(`Explorer: https://explorer.solana.com/tx/${signature}?cluster=testnet`);
}
async function main() {
