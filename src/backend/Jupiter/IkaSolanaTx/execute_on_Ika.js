...
const RECIPIENT = new PublicKey("GR5HAedxPBDnV8PUb4dywaHHxPvTpV7wAXNk9MyDYBuH" // replace with actual recipient
);
...
async function main() {
    ...
    const publicKey = await publicKeyFromDWalletOutput(Curve.ED25519, dWalletPublicOutput);
    ...
    {
        const { tx, messageBytes } = await buildUnsignedSOLTransfer(connection, solanaFromPubkey, RECIPIENT, SOL_TRANSFER_LAMPORTS);
        // 1) Create sign request on Ika
        const { signObjectId } = await ikaSignBytes(suiClient, ikaClient, messageBytes, executeTransaction, signerAddress);
        if (!signObjectId) {
    ...
    console.log("\n=== Memo via Ika ===");
    {
        ...
        const { tx, messageBytes } = await buildUnsignedMemoTx(connection, solanaFromPubkey, memoText);
        const { signObjectId } = await ikaSignBytes(suiClient, ikaClient, messageBytes, executeTransaction, signerAddress);
        if (!signObjectId) {
            throw new Error("Sign object id not found in transaction results");
        }
