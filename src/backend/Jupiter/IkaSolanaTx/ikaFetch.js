 ...
 */
export async function fetchIkaSignature(ikaClient, signObjectId) {
    console.log("[Debug] Fetching sign object:", signObjectId);
    const sign = await ikaClient.getSignInParticularState(signObjectId, Curve.ED25519, SignatureAlgorithm.EdDSA, "Completed");
    console.log("[Debug] Sign state:", sign.state?.$kind);
    console.log("[Debug] Sign object full:", JSON.stringify(sign, (key, value) => value instanceof Uint8Array ? `Uint8Array(${value.length}): ${Buffer.from(value).toString('hex').slice(0, 64)}...` : value, 2));
