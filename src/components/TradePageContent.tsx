...
function OrderEntry({ selectedMarket, livePrice, onEvmAddressChange }: OrderEntryProps) {
  ...
  // Multi-leg routing state
  const VENUES = ["Hyperliquid", "Aster"] as const;
  type Venue = typeof VENUES[number];
  const [legs, setLegs] = useState<{ venue: Venue; size: string }[]>([
    { venue: "Hyperliquid", size: "" },
  ]);

  function addLeg() {
    if (legs.length >= VENUES.length) return;
    const usedVenues = new Set(legs.map((l) => l.venue));
    const next = VENUES.find((v) => !usedVenues.has(v));
    if (next) setLegs([...legs, { venue: next, size: "" }]);
  }
  ...
  function updateLeg(index: number, field: "venue" | "size", value: string) {
    setLegs(legs.map((l, i) => (i === index ? { ...l, [field]: value } : l)));
  }
    ...
    <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-1.5">
                <input
                  ...
                  className="flex-1 min-w-0 px-2.5 py-1.5 bg-zinc-800 border border-zinc-700 rounded-lg text-[11px] text-zinc-100 font-mono placeholder-zinc-600 focus:outline-none focus:ring-0 focus:border-zinc-500 disabled:opacity-50"
                ...
                <>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2.5 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-zinc-100 font-mono placeholder-zinc-600 focus:outline-none focus:ring-0 focus:border-zinc-500 disabled:opacity-50"
                      placeholder="10.00"
                    />
                  </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="flex items-center gap-1.5 text-zinc-400 text-[12px]">
            <GitMerge size={11} className="text-indigo-400" />
            Route Legs
          </label>
          {legs.length < VENUES.length && (
            <button
              onClick={addLeg}
        ...
        <div className="space-y-2">
          {legs.map((leg, i) => (
            <div
              key={i}
              className="flex items-center gap-2 px-3 py-2.5 bg-zinc-800/40 border border-zinc-700/30 rounded-lg"
            >
              {/* Venue Selector */}
              <select
                value={leg.venue}
                onChange={(e) => updateLeg(i, "venue", e.target.value)}
                className="bg-zinc-800 border border-zinc-700/50 rounded-md px-2 py-1.5 text-[11px] text-zinc-200 font-medium focus:outline-none focus:ring-0 focus:border-zinc-500 appearance-none cursor-pointer"
              >
                {VENUES.map((v) => (
              ...
              {/* Size Input */}
              <div className="flex-1 flex items-center bg-zinc-800/60 border border-zinc-700/50 rounded-md h-8">
                <input
                  type="text"
                  value={leg.size}
                  onChange={(e) => updateLeg(i, "size", e.target.value)}
                  placeholder="Size"
                  className="flex-1 bg-transparent px-2 text-zinc-100 font-mono text-[11px] outline-none focus:ring-0 placeholder-zinc-700"
                />
                <span className="text-zinc-600 text-[10px] pr-2 font-mono">{sizeUnit}</span>
              </div>
              {/* Remove */}
              {legs.length > 1 && (
                <button
        ...
        {legs.length > 1 && (
          <div className="flex items-center gap-1.5 mt-1.5 px-1">
            ...
            <span className="text-[10px] text-zinc-600">
              Order will be split across {legs.length} venues
            </span>
          </div>
        )}
      </div>
...
export default function TradePageContent() {
  ...
  return (
    <div className="min-h-screen bg-[#020408] font-[family-name:var(--font-geist-sans)]">
      {/* ── TOP NAV ── */}
      <header className="sticky top-0 z-40 bg-[#020408]/80 backdrop-blur-xl border-b border-white/[0.06]">
      ...
      <main className="max-w-[1600px] mx-auto px-6 sm:px-8 py-6 space-y-6">
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 lg:w-[75%] bg-zinc-900/50 border border-zinc-700/40 rounded-2xl overflow-hidden shadow-lg">
